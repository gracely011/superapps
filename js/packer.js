...
// Pemulihan mutlak native ES6 Map & Set bawaan peramban agar Tailwind CDN & library modern berjalan stabil
if (_supertoolsNativeMap && typeof window !== 'undefined') window.Map = _supertoolsNativeMap;
if (_supertoolsNativeSet && typeof window !== 'undefined') window.Set = _supertoolsNativeSet;
if (typeof globalThis !== 'undefined') {
  if (_supertoolsNativeMap) globalThis.Map = _supertoolsNativeMap;
  if (_supertoolsNativeSet) globalThis.Set = _supertoolsNativeSet;
}

// ==========================================
// SUPERTOOLS JAVASCRIPT PACKER CONTROLLER
// ==========================================

let globalPackerInstance = null;

function getPacker() {
  if (!globalPackerInstance && typeof Packer === 'function') {
    globalPackerInstance = new Packer();
  }
  return globalPackerInstance;
}

const PACKER_SAMPLE = `// Contoh Kode JavaScript untuk Pengujian Packer
...
const r = 7;
const luas = hitungLuasLingkaran(r);
console.log("Luas lingkaran dengan r = " + r + " adalah: " + luas.toFixed(2));
alert("Hasil perhitungan luas: " + luas);`;

function showPackerToast(msg, isSuccess = true) {
  const t = document.getElementById('packer-toast');
  if (!t) return;
...
function updatePackerInputStats() {
  const inputEl = document.getElementById('packer-input');
  const statsEl = document.getElementById('packer-input-stats');
  if (inputEl && statsEl) {
    statsEl.textContent = inputEl.value.length.toLocaleString('id-ID') + ' Karakter';
  }
}

function doPackerProcess() {
  const inputEl = document.getElementById('packer-input');
  const outputEl = document.getElementById('packer-output');
  const base62El = document.getElementById('packer-base62');
  const shrinkEl = document.getElementById('packer-shrink');
  const outputStatsEl = document.getElementById('packer-output-stats');
  const execTimeEl = document.getElementById('packer-exec-time');

  const rawInput = inputEl ? inputEl.value : '';
  if (!rawInput || !rawInput.trim()) {
    showPackerToast('Silakan masukkan kode JavaScript terlebih dahulu!', false);
  ...
  const p = getPacker();
  if (!p) {
    showPackerToast('Engine Packer belum siap!', false);
    return;
  }

  const startTime = performance.now();
  try {
    const isBase62 = base62El ? base62El.checked : true;
    const isShrink = shrinkEl ? shrinkEl.checked : true;
    const packedResult = p.pack(rawInput, isBase62, isShrink);
    const duration = (performance.now() - startTime).toFixed(2);

    if (outputEl) outputEl.value = packedResult;

    const inLen = rawInput.length;
    const outLen = packedResult.length;
    const ratio = inLen > 0 ? (outLen / inLen).toFixed(3) : "1.000";

    if (outputStatsEl) {
      outputStatsEl.textContent = outLen.toLocaleString('id-ID') + ' Karakter (Rasio ' + ratio + ')';
    }
    if (execTimeEl) {
      execTimeEl.innerHTML = '&#9889; Selesai dalam <b>' + duration + ' ms</b> (' + outLen + '/' + inLen + ' B)';
    }
    showPackerToast('Kode berhasil di-pack (Rasio ' + ratio + ')!');
  } catch (err) {
    showPackerToast('Gagal pack script: ' + err.message, false);
  }
}

function doPackerDecode() {
  const outputEl = document.getElementById('packer-output');
  const currentVal = outputEl ? outputEl.value.trim() : '';

  if (!currentVal) {
    showPackerToast('Belum ada kode ter-pack untuk didecode!', false);
    return;
  }

  if (!currentVal.startsWith('eval')) {
    showPackerToast('Kode di panel output bukan berformat eval packer!', false);
    return;
  }

  try {
    let val;
    eval("val = String" + currentVal.slice(4));
    if (outputEl) outputEl.value = val;
    showPackerToast('Berhasil decode kode eval!');
  } catch (err) {
    showPackerToast('Gagal decode: ' + err.message, false);
  }
}

function loadPackerSample() {
  const inputEl = document.getElementById('packer-input');
  if (inputEl) {
    inputEl.value = PACKER_SAMPLE;
    updatePackerInputStats();
    showPackerToast('Contoh kode dimuat!');
    doPackerProcess();
  }
}
...
function clearPacker() {
  ...
  updatePackerInputStats();
  showPackerToast('Area kerja dibersihkan');
}

function copyPackerCode() {
  const outputEl = document.getElementById('packer-output');
  if (!outputEl || !outputEl.value) {
    showPackerToast('Tidak ada kode untuk disalin!', false);
    return;
  }
  navigator.clipboard.writeText(outputEl.value).then(() => {
    showPackerToast('Hasil pack berhasil disalin ke clipboard!');
  }).catch(() => {
    outputEl.select();
    document.execCommand('copy');
    showPackerToast('Hasil pack berhasil disalin!');
  });
}

function pastePackerCode() {
  const inputEl = document.getElementById('packer-input');
  if (!inputEl) return;
  navigator.clipboard.readText().then(text => {
    inputEl.value = text;
    updatePackerInputStats();
    showPackerToast('Kode berhasil ditempel dari clipboard');
  }).catch(() => {
    inputEl.focus();
    showPackerToast('Gunakan Ctrl+V untuk menempel kode', false);
  });
}

function downloadPackerCode() {
  const outputEl = document.getElementById('packer-output');
  if (!outputEl || !outputEl.value) {
    showPackerToast('Tidak ada kode untuk diunduh!', false);
    return;
  }
  ...
  a.href = url;
  a.download = 'packed.js';
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  showPackerToast('File packed.js berhasil diunduh');
}

function handlePackerUpload(file) {
  if (!file) return;
  ...
  reader.onload = e => {
    ...
    if (inputEl) {
      ...
      updatePackerInputStats();
      showPackerToast('File ' + file.name + ' berhasil dimuat');
    }
  };
...
document.addEventListener('DOMContentLoaded', () => {
  const inputEl = document.getElementById('packer-input');
  if (inputEl) {
    inputEl.addEventListener('input', updatePackerInputStats);
